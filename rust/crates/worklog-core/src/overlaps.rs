...
/// Every WORK project's full gap-bridged activity for the day — unlike
/// `day_overlaps`, this is per-project and doesn't require a second project
/// to also be active; it's what the lanes view draws as a faint fill behind
/// each lane's owned block segments (`infer_lanes` picks one owner per
/// minute, so a project's non-owned activity is otherwise invisible).
pub fn day_activity(conn: &Connection, day: NaiveDate) -> Result<Vec<ProjectActivity>> {
...
fn compute_activity(events: &[InferEvent]) -> Vec<ProjectActivity> {
    ...
    by_project
        .into_iter()
        .map(|(project, spans)| ProjectActivity { project, spans })
        .collect()
}
