"""Tempo sync uses blocks with real durations; posts per jira_issue."""
...
def db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    with connect(p) as conn:
        conn.execute(
            """
            INSERT INTO blocks (day, jira_issue, started_at, ended_at,
                                duration_seconds, description, estimated_by)
            VALUES ('2026-04-18', 'ACME-42',
...
def test_sync_day_dry_run_uses_block_duration(db: Path) -> None:
    ...
    assert payload["timeSpentSeconds"] == 2700
...
def test_sync_day_marks_block_after_success(db: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class FakeClient:
        ...
        def post(self, url, json): return FakeResp()
    ...
    with connect(db) as conn:
        tempo_id = conn.execute("SELECT tempo_worklog_id FROM blocks").fetchone()[0]
...
def test_sync_day_skips_blocks_without_ticket(db: Path) -> None:
    from worklog.tempo import sync_day

    with connect(db) as conn:
        conn.execute("UPDATE blocks SET jira_issue = NULL")

    results = sync_day(date(2026, 4, 18), dry_run=True)
    assert len(results) == 1
    assert results[0]["status"] == "skipped"
    assert "no jira_issue" in results[0]["reason"]
